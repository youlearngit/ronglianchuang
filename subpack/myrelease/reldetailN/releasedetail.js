...
import {delBillBusiness,queryBillBusiness,queryRegisterInfo} from '../../../api/url'
const app = getApp()
...
async getloading(){
 ...
 await this.getqueryRegisterInfo()
 
},
 
 getqueryRegisterInfo(){
  return  queryRegisterInfo({OPEN_ID:this.data.OPEN_ID}).then(res => {
      console.log(res);
      if(res.STATUS==1){
 ...
 getDetail(){
    ...
    let that=this
  return queryBillBusiness({BB_ID:this.data.id,OPEN_ID:wx.getStorageSync('wxlogininfo').OPEN_ID}).then(res => {
      console.log(res);
      if(res.STATUS==1){
       this.setData({
         info:res.LIST[0]
  ...
  delete(){
    delBillBusiness({BB_ID:this.data.info.BB_ID}).then(res => {
      console.log(res);
      if(res.STATUS==1){
        wx.showToast({
          title: '删除成功',
...
/**
  ...
  editfuc:function(){
wx.navigateTo({
  url: '/pages/bill/bill?info='+JSON.stringify(this.data.info)
})
  },
  /**
   * 生命周期函数--监听页面隐藏
