...
import {delBillBusiness,selectCreditBond,deleteCreditBond,delFile} from '../../../api/url'
const app = getApp()
...
Page({
  ...
  getDetail(){
    selectCreditBond({CB_ID:this.data.id,OPEN_ID:wx.getStorageSync('wxlogininfo').OPEN_ID}).then(res => {
      console.log(res);
      if(res.STATUS==1){
       this.setData({
         info:res.LIST[0]
  ...
  delete(){
    return  delFile({CB_ID:this.data.id}).then(res => {
      console.log(res); 
    }).catch(e=>{
      console.log(e)
      wx.showToast({
    ...
    deleteCreditBond({CB_ID:this.data.id}).then(res => {
      console.log(res);
      if(res.STATUS==1){
        wx.showToast({
          title: '删除成功',
