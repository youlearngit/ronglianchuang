...
import {deletePublicFund,selectPublicFundList,delFile} from '../../../api/url'
const app = getApp()
...
Page({
  ...
  getDetail(){
    ...
    selectPublicFundList({PF_ID:this.data.id,OPEN_ID:wx.getStorageSync('wxlogininfo').OPEN_ID}).then(res => {
      console.log(res);
      if(res.STATUS==1){
       this.setData({
         info:res.LIST[0]
  ...
  delete(){
    return  delFile({PF_ID:this.data.info.PF_ID}).then(res => {
      console.log(res); 
    }).catch(e=>{
      console.log(e)
      wx.showToast({
      ...
      deletePublicFund({PF_ID:this.data.info.PF_ID}).then(res => {
      console.log(res);
      if(res.STATUS==1){
        wx.showToast({
          title: '删除成功',
...
/**
  ...
  editfuc:function(){
wx.navigateTo({
  url: '/pages/publicFund/publicFund?info='+JSON.stringify(this.data.info)
})
  },
  /**
   * 生命周期函数--监听页面隐藏
