...
import {queryFinanceBond,delFinanceBond,delFile} from '../../../api/url'
const app = getApp()
...
Page({
  ...
  getDetail(){
    queryFinanceBond({FB_ID:this.data.id,OPEN_ID:wx.getStorageSync('wxlogininfo').OPEN_ID}).then(res => {
      if(res.STATUS==1){
       this.setData({
         info:res.LIST[0]
       })
  ...
  delete(){
    return  delFile({FB_ID:this.data.id}).then(res => {
      console.log(res); 
    }).catch(e=>{
      console.log(e)
      wx.showToast({
    ...
    delFinanceBond({FB_ID:this.data.id}).then(res => {
      console.log(res);
      if(res.STATUS==1){
        wx.showToast({
          title: '删除成功',
