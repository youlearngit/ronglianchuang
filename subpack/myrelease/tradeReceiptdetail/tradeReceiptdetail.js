...
import {deletePublicFund,selectPublicFundList,querySyndicatedLoan,delSyndicatedLoan,queryTradeBorrow,delTradeBorrow,selectTradeReceiptList,deleteTradeReceipt,delFile} from '../../../api/url'
const app = getApp()
...
Page({
  ...
  getDetail(){
    ...
    selectTradeReceiptList({TDR_ID:this.data.id,OPEN_ID:wx.getStorageSync('wxlogininfo').OPEN_ID}).then(res => {
      console.log(res);
      if(res.STATUS==1){
       this.setData({
         info:res.LIST[0]
  ...
  delete(){
  return  delFile({TDR_ID:this.data.info.TDR_ID}).then(res => {
      console.log(res); 
    }).catch(e=>{
      console.log(e)
      wx.showToast({
      ...
      return
    }).then(deleteTradeReceipt({TDR_ID:this.data.info.TDR_ID}).then(res => {
      console.log(res);
      if(res.STATUS==1){
        wx.showToast({
          title: '删除成功',
...
/**
  ...
  editfuc:function(){
wx.navigateTo({
  url: '/pages/tradeReceipt/tradeReceipt?info='+JSON.stringify(this.data.info)
})
  },
  /**
   * 生命周期函数--监听页面隐藏
