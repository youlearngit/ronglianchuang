...
import {deletePublicFund,selectPublicFundList,querySyndicatedLoan,delSyndicatedLoan,queryTradeBorrow,delTradeBorrow,selectTradeDepositList,deleteTradeDeposit,delFile} from '../../../api/url'
const app = getApp()
...
Page({
  ...
  getDetail(){
    ...
    selectTradeDepositList({TD_ID:this.data.id,OPEN_ID:wx.getStorageSync('wxlogininfo').OPEN_ID}).then(res => {
      console.log(res);
      if(res.STATUS==1){
       this.setData({
         info:res.LIST[0]
  ...
  delete(){
    return  delFile({TD_ID:this.data.info.TD_ID}).then(res => {
      console.log(res); 
    }).catch(e=>{
      console.log(e)
      wx.showToast({
    ...
    deleteTradeDeposit({TD_ID:this.data.info.TD_ID}).then(res => {
      console.log(res);
      if(res.STATUS==1){
        wx.showToast({
          title: '删除成功',
...
/**
  ...
  editfuc:function(){
wx.navigateTo({
  url: '/pages/tradeDeposit/tradeDeposit?info='+JSON.stringify(this.data.info)
})
  },
  /**
   * 生命周期函数--监听页面隐藏
