...
import {deletePublicFund,selectPublicFundList,querySyndicatedLoan,delSyndicatedLoan,delFile} from '../../../api/url'
const app = getApp()
...
Page({
  ...
  getDetail(){
    ...
    querySyndicatedLoan({SL_ID:this.data.id,OPEN_ID:wx.getStorageSync('wxlogininfo').OPEN_ID}).then(res => {
      console.log(res);
      if(res.STATUS==1){
       this.setData({
         info:res.LIST[0]
  ...
  delete(){
    return  delFile({SL_ID:this.data.info.SL_ID}).then(res => {
      console.log(res); 
    }).catch(e=>{
      console.log(e)
      wx.showToast({
    ...
    delSyndicatedLoan({SL_ID:this.data.info.SL_ID}).then(res => {
      console.log(res);
      if(res.STATUS==1){
        wx.showToast({
          title: '删除成功',
...
/**
  ...
  editfuc:function(){
wx.navigateTo({
  url: '/pages/syndicatedLoan/syndicatedLoan?info='+JSON.stringify(this.data.info)
})
  },
  /**
   * 生命周期函数--监听页面隐藏
