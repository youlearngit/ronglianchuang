...
import {addOrUpdateCreditBond,addOrUpdateRateBond,addOrUpdateFinanceBond,rlcuploadFile,sensitiveWord,addOrUpdateAbs,addOrUpdateTradeFinance,selectRlcUserInfoList,queryWhiteOrgList} from '../../api/url'
...
go_search_rlc(e){
  ...
  getRATING(e){
    this.setData({
      'creditBond.TF_INTEREST_RATE':e.detail.value
    })
  },

  getAMOUNT(e){
    this.setData({
      'creditBond.TF_AMOUNT':e.detail.value
    })
  },
...
// 上传图片
  ...
  confirmCreditBond(e) { //提交信用债最后点击提交时
    ...
    if (!this.data.creditBond.TF_INTEREST_RATE) {
      wx.showToast({
        title: "请输入利率",
        icon: "none",
        duration: 1500,
    ...
    if (!this.data.creditBond.TF_AMOUNT) {
      wx.showToast({
        title: "请输入金额",
        icon: "none",
        duration: 1500,
  ...
  if (this.data.selectedItems.item8==2 && this.data.PUBLISH_OBJ_CONTENT.length==0) {
    ...
    let data={
      TF_TRANSACTION:this.data.arr[this.data.selectedItems.item1],
      TF_PRODUCT_NATURE:this.data.arr2[this.data.selectedItems.item2],
      
      TF_HAVE_NON_STANDARD:this.data.arr3[this.data.selectedItems.item3],
      
      TF_PUBLISH_OBJ:this.data.arr8[this.data.selectedItems.item8],

      
      TF_PUBLISH_DEADLINE:this.data.fxqxday +'-' + this.data.arr22[this.data.selectedItems.item22],

      

      
      TF_INTEREST_RATE:this.data.creditBond.TF_INTEREST_RATE,
      TF_AMOUNT:this.data.creditBond.TF_AMOUNT,
      

      TF_REMARKS:this.data.creditBond.REMARKS,
      TF_ENCLOSURE:this.data.BB_ENCLOSURE.join(','),
    ...
    if(this.data.TF_ID){
       data.TF_ID=this.data.TF_ID
    }

    console.log(data)
    
    addOrUpdateTradeFinance(data).then(res => {
      console.log(res);
      if(res.STATUS == 1){
        wx.showToast({
                title: '发布成功',
    ...
    if(options.info){
      ...
      // OPEN_ID:wx.getStorageSync('wxlogininfo').OPEN_ID
          ...
          this.data.arr2.map((item2,index2)=>{
            ...
            if(item2==info.TF_PRODUCT_NATURE){
          ...
          this.data.arr3.map((item,index)=>{
            ...
            if(item==info.TF_HAVE_NON_STANDARD){
          ...
          'creditBond.TF_AMOUNT':info.TF_AMOUNT,
          'creditBond.TF_INTEREST_RATE':info.TF_INTEREST_RATE,


          'creditBond.REMARKS':info.TF_REMARKS,
          fbundentub:true,
          entub:true,
          current:info.TF_REMARKS.length,
          TF_ID:info.TF_ID,

          fxqxday:info.TF_PUBLISH_DEADLINE.split('-')[0],
          isEdit:true,
