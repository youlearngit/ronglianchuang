...
import {addOrUpdateCreditBond,addOrUpdateRateBond,addOrUpdateFinanceBond,rlcuploadFile,sensitiveWord,addOrUpdateAbs,addOrUpdateSyndicatedLoan,selectRlcUserInfoList,queryWhiteOrgList} from '../../api/url'
...
Page({
  ...
  data: {
    ...
    tabs: ["信用债", "金融债", "利率债"],
    currentIndex: 0,
    arr: ["资产支持证券（ABS）", "银登中心（资产流转产品）", "资产支持票据（ABN））"],
    arr2: ["上交所", "深交所",'银登中心','银行间市场'],
    arr3: ["公司信贷资产", "个人消费贷款", "个人按揭贷款", "信用卡分期", "类信贷资产","基金资产"],
    arr4: ["优先", "次级","夹层","劣后"],
    ...
    //银行贷款
    INFORMATION_NAME:'',//信息机构发布名称
    PROJECT_NAME:'',//项目名称
    LEADING_BANK_NAME:'',//牵头行名称
    BORROWER_NAME:'',//借款人名称
    TOTAL_SYNDICATION:'',//银团总额
    LEAD_LINE:'',//预计牵头额度
    DISTRIBUTION_QUOTA:'',//预计分销额度DISTRIBUTION_QUOTA
    YIELD:'',  //预计收益率
    PROJECT_CONTACT:'',//项目联系人
    CONTACT_NUMBER:'',//联系电话
    BORROWER_INDUSTRY:'',//借款人所处行业
    INDUSTRY:'',//项目所处行业
    GUARANTEE_INFORMATION:'',//担保信息
    ...
    sensitive_PROJECT_NAME:false,

    sensitive_INFORMATION_NAME:false,
    sensitive_LEADING_BANK_NAME:false,
    sensitive_BORROWER_NAME:false,
    sensitive_TOTAL_SYNDICATION:false,
    sensitive_PROJECT_CONTACT:false,
    sensitive_BORROWER_INDUSTRY:false,
    sensitive_INDUSTRY:false,
    sensitive_GUARANTEE_INFORMATION:false,
    sensitive_PROJECT_PROGRESS:false,
...
go_search_org(e){
  ...
  getRATING(e){
    this.setData({
      'creditBond.RATING':e.detail.value
    })
  },

  getCONTACT_NUMBER(e){
    this.setData({
      'creditBond.CONTACT_NUMBER':e.detail.value
    })
  },

  getAMOUNT(e){
    this.setData({
      'creditBond.AMOUNT':e.detail.value
    })
  },

  //项目进展情况
  getPROJECT_PROGRESS(e){
    sensitiveWord({TEXT:e.detail.value}).then(res => {
      console.log(res)  
      if(res.RESULT!=='OK'){
        this.setData({
          sensitive_PROJECT_PROGRESS:true
        })
      }else{
        this.setData({
          sensitive_PROJECT_PROGRESS:false
    ...
    this.setData({
      'creditBond.PROJECT_PROGRESS':e.detail.value
    })
  },

  //担保信息
  getGUARANTEE_INFORMATION(e){
    sensitiveWord({TEXT:e.detail.value}).then(res => {
      console.log(res)  
      if(res.RESULT!=='OK'){
        this.setData({
          sensitive_GUARANTEE_INFORMATION:true
        })
      }else{
        this.setData({
          sensitive_GUARANTEE_INFORMATION:false
    ...
    this.setData({
      'creditBond.GUARANTEE_INFORMATION':e.detail.value
    })
  },
  //项目所处行业
  getINDUSTRY(e){
    ...
    this.setData({
      'creditBond.INDUSTRY':e.detail.value
    })
  },
  //借款人所处行业
  getBORROWER_INDUSTRY(e){
    sensitiveWord({TEXT:e.detail.value}).then(res => {
      console.log(res)  
      if(res.RESULT!=='OK'){
        this.setData({
          sensitive_BORROWER_INDUSTRY:true
        })
      }else{
        this.setData({
          sensitive_BORROWER_INDUSTRY:false
    ...
    this.setData({
      'creditBond.BORROWER_INDUSTRY':e.detail.value
    })
  },

  //项目联系人
  getPROJECT_CONTACT(e){
    sensitiveWord({TEXT:e.detail.value}).then(res => {
      console.log(res)  
      if(res.RESULT!=='OK'){
        this.setData({
          sensitive_PROJECT_CONTACT:true
        })
      }else{
        this.setData({
          sensitive_PROJECT_CONTACT:false
    ...
    this.setData({
      'creditBond.PROJECT_CONTACT':e.detail.value
    })
  },
  ...
  getYIELD(e){
    this.setData({
      'creditBond.YIELD':e.detail.value
    })
  },
//预计分销额度
  getDISTRIBUTION_QUOTA(e){
    this.setData({
      'creditBond.DISTRIBUTION_QUOTA':e.detail.value
    })
  },
  //预计牵头额度
  getLEAD_LINE(e){
    this.setData({
      'creditBond.LEAD_LINE':e.detail.value
    })
  },

  //银团总额
  getTOTAL_SYNDICATION(e){
    sensitiveWord({TEXT:e.detail.value}).then(res => {
      console.log(res)  
      if(res.RESULT!=='OK'){
        this.setData({
          sensitive_TOTAL_SYNDICATION:true
        })
      }else{
        this.setData({
          sensitive_TOTAL_SYNDICATION:false
    ...
    this.setData({
      'creditBond.TOTAL_SYNDICATION':e.detail.value
    })
  },
  ...
  getBORROWER_NAME(e){
    ...
    this.setData({
      'creditBond.BORROWER_NAME':e.detail.value
    })
  },

  //牵头行名称
  getLEADING_BANK_NAME(e){
  sensitiveWord({TEXT:e.detail.value}).then(res => {
    console.log(res)  
    if(res.RESULT!=='OK'){
      this.setData({
        sensitive_LEADING_BANK_NAME:true
      })
    }else{
      this.setData({
        sensitive_LEADING_BANK_NAME:false
  ...
  this.setData({
    'creditBond.LEADING_BANK_NAME':e.detail.value
  })
},
...
getPROJECT_NAME(e){
    ...
    this.setData({
      'creditBond.PROJECT_NAME':e.detail.value
    })
  },

  //信息机构发布名称
  getINFORMATION_NAME(e){
    sensitiveWord({TEXT:e.detail.value}).then(res => {
      console.log(res)  
      if(res.RESULT!=='OK'){
        this.setData({
          sensitive_INFORMATION_NAME:true
        })
      }else{
        this.setData({
          sensitive_INFORMATION_NAME:false
    ...
    this.setData({
      'creditBond.INFORMATION_NAME':e.detail.value
    })
  },
  ...
  getpublishscale(e){
    this.setData({
      'creditBond.ISSUING_SCALE':e.detail.value
    })
  },
  getpublisher(e){
     this.setData({
...
// 上传图片
  ...
  confirmCreditBond(e) { //提交信用债最后点击提交时
    ...
    if(this.data.sensitive_INFORMATION_NAME){
      wx.showToast({
          title: '您输入的信息机构发布名称含有敏感词',
          icon: 'none'
        })
      return
   }

    if(this.data.sensitive_PROJECT_NAME){
      wx.showToast({
          title: '您输入的项目名称含有敏感词',
          icon: 'none'
        })
   ...
   if(this.data.sensitive_LEADING_BANK_NAME){
    wx.showToast({
        title: '您输入的牵头行名称含有敏感词',
        icon: 'none'
      })
    return
 }

 if(this.data.sensitive_BORROWER_NAME){
  wx.showToast({
      title: '您输入的借款人名称含有敏感词',
      icon: 'none'
    })
  return
}

if(this.data.sensitive_TOTAL_SYNDICATION){
  wx.showToast({
      title: '您输入的银团总额含有敏感词',
      icon: 'none'
    })
...
if(this.data.sensitive_PROJECT_CONTACT){
  wx.showToast({
      title: '您输入的项目联系人含有敏感词',
      icon: 'none'
    })
  return
}

if(this.data.sensitive_BORROWER_INDUSTRY){
  wx.showToast({
      title: '您输入的借款人所处行业含有敏感词',
      icon: 'none'
    })
  return
}

if(this.data.sensitive_INDUSTRY){
  wx.showToast({
      title: '您输入的项目所处行业含有敏感词',
      icon: 'none'
    })
  return
}
if(this.data.sensitive_GUARANTEE_INFORMATION){
  wx.showToast({
      title: '您输入的担保信息含有敏感词',
      icon: 'none'
    })
  ...
  if(this.data.sensitive_PROJECT_PROGRESS){
    wx.showToast({
        title: '您输入的项目进展情况含有敏感词',
        icon: 'none'
      })
    return
    }
    ...
    if (!this.data.creditBond.INFORMATION_NAME) {
      wx.showToast({
        title: "请输入信息机构发布名称",
        icon: "none",
        duration: 1500,
    ...
    if (!this.data.creditBond.PROJECT_NAME) {
      wx.showToast({
        title: "请输入项目名称",
        icon: "none",
        duration: 1500,
    ...
    if (!this.data.creditBond.LEADING_BANK_NAME) {
      wx.showToast({
        title: "请输入牵头行名称",
        icon: "none",
        duration: 1500,
    ...
    if (!this.data.creditBond.BORROWER_NAME) {
      wx.showToast({
        title: "请输入借款人名称",
        icon: "none",
        duration: 1500,
    ...
    if (!this.data.creditBond.TOTAL_SYNDICATION) {
      wx.showToast({
        title: "请输入银团总额",
        icon: "none",
        duration: 1500,
    ...
    if (!this.data.creditBond.LEAD_LINE) {
      wx.showToast({
        title: "请输入预计牵头额度",
        icon: "none",
        duration: 1500,
    ...
    if (!this.data.creditBond.DISTRIBUTION_QUOTA) {
      wx.showToast({
        title: "请输入预计分销额度",
        icon: "none",
        duration: 1500,
    ...
    if (!this.data.creditBond.PROJECT_CONTACT) {
      wx.showToast({
        title: "请输入项目联系人",
        icon: "none",
        duration: 1500,
  ...
  if (this.data.selectedItems.item8==2 && this.data.PUBLISH_OBJ_CONTENT.length==0) {
    ...
    let data={
      PUBLISH_OBJ:this.data.arr8[this.data.selectedItems.item8],//发布对象
      
      TERM:this.data.fxqxday +'-' + this.data.arr22[this.data.selectedItems.item22],//期限

      INFORMATION_NAME:this.data.creditBond.INFORMATION_NAME,
      PROJECT_NAME:this.data.creditBond.PROJECT_NAME,
      LEADING_BANK_NAME:this.data.creditBond.LEADING_BANK_NAME,
      BORROWER_NAME:this.data.creditBond.BORROWER_NAME,
      TOTAL_SYNDICATION:this.data.creditBond.TOTAL_SYNDICATION,
      LEAD_LINE:this.data.creditBond.LEAD_LINE,
      DISTRIBUTION_QUOTA:this.data.creditBond.DISTRIBUTION_QUOTA,
      YIELD:this.data.creditBond.YIELD,
      PROJECT_CONTACT:this.data.creditBond.PROJECT_CONTACT,
      CONTACT_NUMBER:this.data.creditBond.CONTACT_NUMBER,
      BORROWER_INDUSTRY:this.data.creditBond.BORROWER_INDUSTRY,
      INDUSTRY:this.data.creditBond.INDUSTRY,
      GUARANTEE_INFORMATION:this.data.creditBond.GUARANTEE_INFORMATION,
      PROJECT_PROGRESS:this.data.creditBond.PROJECT_PROGRESS,
    ...
    if(this.data.SL_ID){
       data.SL_ID=this.data.SL_ID
    }

    console.log(data)
    
    addOrUpdateSyndicatedLoan(data).then(res => {
      console.log(res);
      if(res.STATUS == 1){
        wx.showToast({
                title: '发布成功',
    ...
    if(options.info){
      ...
      if(info.SL_ID){
        ...
        let selectedItems={
          ...
          'creditBond.INFORMATION_NAME':info.INFORMATION_NAME,
          'creditBond.PROJECT_NAME':info.PROJECT_NAME,
          'creditBond.LEADING_BANK_NAME':info.LEADING_BANK_NAME,
          'creditBond.BORROWER_NAME':info.BORROWER_NAME,
          'creditBond.TOTAL_SYNDICATION':info.TOTAL_SYNDICATION,
          'creditBond.LEAD_LINE':info.LEAD_LINE,
          'creditBond.DISTRIBUTION_QUOTA':info.DISTRIBUTION_QUOTA,
          'creditBond.YIELD':info.YIELD,
          'creditBond.PROJECT_CONTACT':info.PROJECT_CONTACT,
          'creditBond.CONTACT_NUMBER':info.CONTACT_NUMBER,
          'creditBond.BORROWER_INDUSTRY':info.BORROWER_INDUSTRY,
          'creditBond.INDUSTRY':info.INDUSTRY,
          'creditBond.GUARANTEE_INFORMATION':info.GUARANTEE_INFORMATION,
          'creditBond.PROJECT_PROGRESS':info.PROJECT_PROGRESS,
