...
import {addOrUpdateCreditBond,addOrUpdateRateBond,addOrUpdateFinanceBond,rlcuploadFile,sensitiveWord,addOrUpdateAbs,addOrUpdateTradeFinance,addOrUpdateTradeBorrow,delFile,selectRlcUserInfoList,queryWhiteOrgList} from '../../api/url'
...
Page({
  ...
  data: {
    ...
    arr21:["荷兰式","混合式","美式"],
    arr22: ["O/N", "7D", "14D",'1M','3M','6M','9M','1Y','自定义'],
...
go_search_org(e){
  ...
  getRATING(e){
    this.setData({
      'creditBond.INTEREST_RATE':e.detail.value
    })
  },
  ...
  getzjqxday(e){
   ...
   getCURRENCY(e){
    this.setData({
      'creditBond.CURRENCY':e.detail.value
    })
   },
  ...
  removeImage(e) {
    ...
    console.log(this.data.BB_ENCLOSURE[idx].split('rlc/')[1])

    // delFile({fileName:this.data.BB_ENCLOSURE[idx].split('rlc/')[1]}).then(res => {
    //   console.log(res); 
    // }).catch(e=>{
    //   console.log(e)
    //   wx.showToast({
...
// 上传图片
  ...
  confirmCreditBond(e) { //提交信用债最后点击提交时
    ...
    if (this.data.selectedItems.item22==8 && !this.data.fxqxday) {
      wx.showToast({
        title: "请输入项目期限",
        icon: "none",
        duration: 1500,
    ...
    if (!this.data.creditBond.INTEREST_RATE) {
      wx.showToast({
        title: "请输入利率",
        icon: "none",
        duration: 1500,
    ...
    if (!this.data.creditBond.CURRENCY && this.data.selectedItems.item2==2) {
      wx.showToast({
        title: "请输入币种",
        icon: "none",
        duration: 1500,
  ...
  if (this.data.selectedItems.item8==2 && this.data.PUBLISH_OBJ_CONTENT.length==0) {
    ...
    let data={
      DIRECTION:this.data.arr[this.data.selectedItems.item1],
      // DEADLINE:this.data.fxqxday +'-' + this.data.arr22[this.data.selectedItems.item22],

      // CURRENCY:this.data.creditBond.CURRENCY+'-' + this.data.arr2[this.data.selectedItems.item2],
      DEADLINE:this.data.selectedItems.item22==8 ? this.data.fxqxday : this.data.arr22[this.data.selectedItems.item22],

      CURRENCY:this.data.selectedItems.item2==2 ? this.data.creditBond.CURRENCY : this.data.arr2[this.data.selectedItems.item2],

      PUBLISH_OBJ:this.data.arr8[this.data.selectedItems.item8],
   
      INTEREST_RATE:this.data.creditBond.INTEREST_RATE,
    ...
    if(this.data.TB_ID){
       data.TB_ID=this.data.TB_ID
    }

    console.log(data)
    
    addOrUpdateTradeBorrow(data).then(res => {
      console.log(res);
      if(res.STATUS == 1){
        wx.showToast({
                title: '发布成功',
    ...
    if(options.info){
      ...
      if(info.TB_ID){
        ...
        let selectedItems={
          ...
          // this.data.arr2.forEach((item,index)=>{
            //arr2: ["人民币", "美元",'自定义'],
            if(info.CURRENCY=='人民币'){
              selectedItems.item2=0    
            }else if(info.CURRENCY=='美元'){
              selectedItems.item2=1
            }else{
              selectedItems.item2=2
              this.setData({
                'creditBond.CURRENCY':info.CURRENCY
              })
            }
          // })
          ...
          //   if(item==info.DEADLINE){
              
          //     selectedItems.item22=index
               
          //   }else{
          //     selectedItems.item22=8
          //    this.setData({
          //     fxqxday:info.DEADLINE
          //    })
          //   }
          // })
          for(let i=0;i<this.data.arr22.length;i++){
            if(this.data.arr22[i]==info.DEADLINE){
              selectedItems.item22=i
              break
            }else{
                  selectedItems.item22=8
                 this.setData({
          ...
          'creditBond.INTEREST_RATE':info.INTEREST_RATE,
          ...
          // 'creditBond.CURRENCY':selectedItems.item2==2 ? info.CURRENCY : '',
