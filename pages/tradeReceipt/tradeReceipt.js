...
import {addOrUpdateCreditBond,addOrUpdateRateBond,addOrUpdateFinanceBond,rlcuploadFile,sensitiveWord,addOrUpdateAbs,addOrUpdateTradeReceipt,selectRlcUserInfoList,queryWhiteOrgList} from '../../api/url'
...
go_search_org(e){
  ...
  choose(e) { //单选
    ...
    if(this.data.selectedItems.item1===0){
       this.setData({
         arr2:['发行','认购']
       })
    }
...
// 上传图片
  ...
  confirmCreditBond(e) { //提交信用债最后点击提交时
    ...
    if (!this.data.creditBond.PUBLISHER_DATE) {
      wx.showToast({
        title: "请输入发行日",
        icon: "none",
        duration: 1500,
  ...
  if (this.data.selectedItems.item8==2 && this.data.PUBLISH_OBJ_CONTENT.length==0) {
    ...
    let data={
      MARKET:this.data.arr[this.data.selectedItems.item1],
      DIRECTION:this.data.arr2[this.data.selectedItems.item2],
      
      PUBLISH_TYPE:this.data.arr3[this.data.selectedItems.item3],
     
      MAIN_GRADE:this.data.arr4[this.data.selectedItems.item4],
      PUBLISH_OBJ:this.data.arr8[this.data.selectedItems.item8],
      
      INTEREST_RATE:this.data.creditBond.INTEREST_RATE,
      
      DEADLINE:this.data.arr22[this.data.selectedItems.item22],//期限

    
      ISSUE_DATE:this.data.creditBond.PUBLISHER_DATE,//发行日
    ...
    if(this.data.TDR_ID){
       data.TDR_ID=this.data.TDR_ID
    }

    console.log(data)
    
    addOrUpdateTradeReceipt(data).then(res => {
      console.log(res);
      if(res.STATUS == 1){
        wx.showToast({
                title: '发布成功',
      ...
      if(info.TDR_ID){
          ...
          this.data.arr.map((item,index)=>{
            if(item==info.MARKET){
              ...
              if(selectedItems.item1===0){
                this.setData({
                  arr2:['发行','认购']
                })
             }
             if(selectedItems.item1===1){
               this.setData({
                 arr2:['卖出','买入']
               })
            }
