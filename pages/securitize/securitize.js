...
import {addOrUpdateCreditBond,addOrUpdateRateBond,addOrUpdateFinanceBond,rlcuploadFile,sensitiveWord,addOrUpdateAbs,selectRlcUserInfoList,queryWhiteOrgList} from '../../api/url'
...
addfc(){
this.data.layered.push({
    PRODUCT_LAYERING:'',//分层   
    AMOUNT:'',//金额
    RATING:'',//评级情况       
})
this.setData({
  layered:this.data.layered
})
},
go_search_rlc(e){
  console.log(e)
...
go_search_org(e){
  ...
  getRATING(e){
    this.data.layered[e.currentTarget.dataset.index].RATING=e.detail.value
    this.setData({
      layered:this.data.layered
    })
  },
  ...
  getAMOUNT(e){
    console.log(e)
    this.data.layered[e.currentTarget.dataset.index].AMOUNT=e.detail.value
    this.setData({
      layered:this.data.layered
    })
  },
  ...
  deletefc(e){
   this.data.layered.splice(e.currentTarget.dataset.index,1)
   this.setData({
     layered:this.data.layered
   })
  },
  choosefc(e) { //单选
    console.log(e)
    console.log(this.data.arr4[e.currentTarget.dataset.item])
    this.data.layered[e.currentTarget.dataset.item].PRODUCT_LAYERING=this.data.arr4[e.detail.value]
    this.setData({
     layered:this.data.layered
    });
...
// 上传图片
  ...
  confirmCreditBond(e) { //提交信用债最后点击提交时
    ...
    if (!this.data.creditBond.ISSUING_SCALE) {
      wx.showToast({
        title: "请输入发行规模",
        icon: "none",
        duration: 1500,
  ...
  for(let i=0;i<this.data.layered.length;i++){
    if(!this.data.layered[i].PRODUCT_LAYERING){
      wx.showToast({
        title: '请选择产品分层',
        icon:'none'
      })
  ...
  if (this.data.selectedItems.item8==2 && this.data.PUBLISH_OBJ_CONTENT.length==0) {
    ...
    let data={
      PRODUCT_TYPE:this.data.arr[this.data.selectedItems.item1],
      LISTING_ORGANIZATION:this.data.arr2[this.data.selectedItems.item2],
      
      BASIC_TYPE:this.data.arr3[this.data.selectedItems.item3],
     
      PRODUCT_LAYERING:this.data.arr4[this.data.selectedItems.item4],
      PUBLISH_OBJ:this.data.arr8[this.data.selectedItems.item8],
      
      PROJECT_TERM:this.data.fxqxday +'-' + this.data.arr22[this.data.selectedItems.item22],
      INCOME_RANGE:this.data.creditBond.rate_start+'-'+this.data.creditBond.rate_end,


      PROJECT_NAME:this.data.creditBond.PROJECT_NAME,
      ISSUING_SCALE:this.data.creditBond.ISSUING_SCALE, 
      ISSUE_DATE:this.data.creditBond.PUBLISHER_DATE,
      DUE_DATE:this.data.creditBond.PAY_DATE,
      CLOSING_DATE:this.data.creditBond.LISTED_DATE,
      AMOUNT:this.data.creditBond.AMOUNT,
      RATING:this.data.creditBond.RATING,
    ...
    data.ABS_LIST=this.data.layered
    if(this.data.ABS_ID){
       data.ABS_ID=this.data.ABS_ID
    }

    console.log(data)
    
    addOrUpdateAbs(data).then(res => {
      console.log(res);
      if(res.STATUS == 1){
        wx.showToast({
                title: '发布成功',
...
onLoad: function (options) {
  ...
  this.getQueryWhiteOrgList()
    ...
    if(options.info){
      ...
      // PROJECT_NAME:this.data.creditBond.PROJECT_NAME,
      // PROJECT_TERM:this.data.fxqxday +'-' + this.data.arr22[this.data.selectedItems.item22],
      // ISSUING_SCALE:this.data.ISSUING_SCALE,
      // INCOME_RANGE:this.data.creditBond.rate_start+'-'+this.data.creditBond.rate_end,
      ...
      // OPEN_ID:wx.getStorageSync('wxlogininfo').OPEN_ID
          ...
          this.data.arr2.map((item2,index2)=>{
            ...
            if(item2==info.LISTING_ORGANIZATION){
          ...
          'creditBond.ISSUING_SCALE':info.ISSUING_SCALE, 
          'creditBond.PUBLISHER_DATE':info.ISSUE_DATE,
          'creditBond.PAY_DATE':info.DUE_DATE,
          'creditBond.LISTED_DATE':info.CLOSING_DATE,
          'creditBond.AMOUNT':info.AMOUNT,
          'creditBond.RATING':info.RATING,
          ...
          fxqxday:info.PROJECT_TERM.split('-')[0],
          isEdit:true,
           
          layered:info.ABS_LIST
          }) 
          ...
          if(info.INCOME_RANGE){
            this.setData({
              'creditBond.rate_start':info.INCOME_RANGE.split('-')[0],
              'creditBond.rate_end':info.INCOME_RANGE.split('-')[1],
            })
          }   
      }
