...
import {selectAssetCustodyInfo} from '../../api/url'
...
Page({
  ...
  choose_up_down(e){
    this.data.billBusiness[e.currentTarget.dataset.index].ischecked= !this.data.billBusiness[e.currentTarget.dataset.index].ischecked
   this.setData({
     billBusiness:this.data.billBusiness
   })
  },
 
  goDetail(e){  
  let anchor='anchor'+e.currentTarget.dataset.item.AC_SORT
  console.log(anchor)
   wx.navigateTo({
     url: '/pages/custodydetail/custodydetail?anchor='+anchor
   })
  },
  ...
  loadMoreData:function(){
    ...
    selectAssetCustodyInfo(data).then(res => {
      console.log(res);
      if(res.LIST){
        var list = res.LIST;
        console.log(list);
