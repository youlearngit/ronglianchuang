...
import {addOrUpdateCreditBond,addOrUpdateRateBond,addOrUpdateFinanceBond,rlcuploadFile,sensitiveWord,addOrUpdateAbs,addOrUpdateTradeFinance,addOrUpdateTradeDeposit,selectRlcUserInfoList,queryWhiteOrgList} from '../../api/url'
...
// 上传图片
  ...
  if (this.data.selectedItems.item8==2 && this.data.PUBLISH_OBJ_CONTENT.length==0) {
    ...
    if(this.data.TD_ID){
       data.TD_ID=this.data.TD_ID
    }

    console.log(data)
    
    addOrUpdateTradeDeposit(data).then(res => {
      console.log(res);
      if(res.STATUS == 1){
        wx.showToast({
                title: '发布成功',
    ...
    if(options.info){
      ...
      if(info.TD_ID){
        ...
        let selectedItems={
          ...
          // this.data.arr2.map((item,index)=>{
   
          
          //   if(item==info.CURRENCY){
          //     selectedItems.item2=index
          //   }else{
