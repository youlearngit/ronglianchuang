...
import {addOrUpdateCreditBond,addOrUpdateRateBond,addOrUpdateFinanceBond,rlcuploadFile,sensitiveWord,addOrUpdateAbs,addOrUpdatePublicFund,selectRlcUserInfoList,queryWhiteOrgList} from '../../api/url'
...
Page({
  ...
  data: {
    ...
    //资产证券化
    PF_PRODUCT_NAME:'',//产品名称
    ISSUING_SCALE:'',//发行规模
    AMOUNT:'',//金额
    RATING:'',//评级情况

    PF_ADMINISTRATOR:'',//管理人
    PF_CUSTODIAN:'',//托管人
    PF_PUBLISH_DEADLINE:'',//期限
    PF_SCALE:'',//预期规模
    PF_PURCHASE_AMOUNT:'',//起购金额
    PF_INVESTMENT_SCOPE:'',//投资范围
    ...
    sensitive_PF_PRODUCT_NAME:false,

    sensitive_PF_ADMINISTRATOR:false,


    sensitive_PF_CUSTODIAN:false,

    sensitive_PF_PUBLISH_DEADLINE:false,

    sensitive_PF_SCALE:false,

    sensitive_PF_PURCHASE_AMOUNT:false,

    sensitive_PF_INVESTMENT_SCOPE:false,
...
//起购金额
getPF_PURCHASE_AMOUNT(e){
    this.setData({
      'creditBond.PF_PURCHASE_AMOUNT':e.detail.value
    })
  },

  //预期规模
  getPF_INVESTMENT_SCOPE(e){
    sensitiveWord({TEXT:e.detail.value}).then(res => {
      console.log(res)  
      if(res.RESULT!=='OK'){
        this.setData({
          sensitive_PF_INVESTMENT_SCOPE:true
        })
      }else{
        this.setData({
          sensitive_PF_INVESTMENT_SCOPE:false
    ...
    this.setData({
      'creditBond.PF_INVESTMENT_SCOPE':e.detail.value
    })
  },
  ...
  getPF_SCALE(e){
    ...
    this.setData({
      'creditBond.PF_SCALE':e.detail.value
    })
  },

  //期限
  getPF_PUBLISH_DEADLINE(e){
    sensitiveWord({TEXT:e.detail.value}).then(res => {
      console.log(res)  
      if(res.RESULT!=='OK'){
        this.setData({
          sensitive_PF_PUBLISH_DEADLINE:true
        })
      }else{
        this.setData({
          sensitive_PF_PUBLISH_DEADLINE:false
    ...
    this.setData({
      'creditBond.PF_PUBLISH_DEADLINE':e.detail.value
    })
  },
  ...
  getPF_CUSTODIAN(e){
    ...
    this.setData({
      'creditBond.PF_CUSTODIAN':e.detail.value
    })
  },


  //管理人
  getPF_ADMINISTRATOR(e){
    sensitiveWord({TEXT:e.detail.value}).then(res => {
      console.log(res)  
      if(res.RESULT!=='OK'){
        this.setData({
          sensitive_PF_ADMINISTRATOR:true
        })
      }else{
        this.setData({
          sensitive_PF_ADMINISTRATOR:false
    ...
    this.setData({
      'creditBond.PF_ADMINISTRATOR':e.detail.value
    })
  },
  ...
  getPROJECT_NAME(e){
    sensitiveWord({TEXT:e.detail.value}).then(res => {
      ...
      if(res.RESULT!=='OK'){
        this.setData({
          sensitive_PF_PRODUCT_NAME:true
        })
      }else{
        this.setData({
          sensitive_PF_PRODUCT_NAME:false
    ...
    this.setData({
      'creditBond.PF_PRODUCT_NAME':e.detail.value
    })
  },
  ...
  getPayDate(e){
    this.setData({
      'creditBond.PF_DUE_DATE':e.detail.value
    })
  },
  bindPublisher(e){
    console.log(e)
    this.setData({
      'creditBond.PF_SET_UP_DATE':e.detail.value
    })
...
// 上传图片
  ...
  confirmCreditBond(e) { //提交信用债最后点击提交时
    ...
    if(this.data.sensitive_PF_PRODUCT_NAME){
      wx.showToast({
          title: '您输入的产品名称含有敏感词',
          icon: 'none'
        })
   ...
   if(this.data.sensitive_PF_ADMINISTRATOR){
    wx.showToast({
        title: '您输入的管理人含有敏感词',
        icon: 'none'
      })
 ...
 if(this.data.sensitive_PF_CUSTODIAN){
  wx.showToast({
      title: '您输入的托管人含有敏感词',
      icon: 'none'
    })
...
if(this.data.sensitive_PF_PUBLISH_DEADLINE){
  wx.showToast({
      title: '您输入的期限含有敏感词',
      icon: 'none'
    })
...
if(this.data.sensitive_PF_INVESTMENT_SCOPE){
  wx.showToast({
      title: '您输入的投资范围含有敏感词',
      icon: 'none'
    })
...
if (!this.data.creditBond.PF_PRODUCT_NAME) {
  wx.showToast({
    title: "请输入产品名称",
    icon: "none",
    duration: 1500,
  ...
  return;
    ...
    if (!this.data.creditBond.PF_ADMINISTRATOR) {
      wx.showToast({
        title: "请输入管理人",
        icon: "none",
        duration: 1500,
    ...
    if (!this.data.creditBond.PF_CUSTODIAN) {
      wx.showToast({
        title: "请输入托管人",
        icon: "none",
        duration: 1500,
    ...
    // if (!this.data.creditBond.PF_PUBLISH_DEADLINE) {
    //   wx.showToast({
    //     title: "请输入期限",
    //     icon: "none",
    //     duration: 1500,
    ...
    if (!this.data.creditBond.PF_SCALE) {
      wx.showToast({
        title: "请输入预期规模",
        icon: "none",
        duration: 1500,
    ...
    if (!this.data.creditBond.PF_PURCHASE_AMOUNT) {
      wx.showToast({
        title: "请输入起购金额",
        icon: "none",
        duration: 1500,
    ...
    if (!this.data.creditBond.PF_SET_UP_DATE) {
      wx.showToast({
        title: "请选择成立日期",
        icon: "none",
        duration: 1500,
    ...
    // if(!this.data.creditBond.mini_rate || !this.data.creditBond.high_rate){
    //   wx.showToast({
    //     title: '请输入正确的开放期',
    //     icon: "none",
    //     duration: 1500,
    ...
    // if (!this.data.creditBond.PF_INVESTMENT_SCOPE) {
    //   wx.showToast({
    //     title: "请输入投资范围",
    //     icon: "none",
    //     duration: 1500,
  ...
  if (this.data.selectedItems.item8==2 && this.data.PUBLISH_OBJ_CONTENT.length==0) {
    ...
    let data={
      PF_INVESTMENT_TYPE:this.data.arr[this.data.selectedItems.item1],//投资类型
      PF_OPERATION_MODE:this.data.arr2[this.data.selectedItems.item2],//运作方式
     
      PF_RETURN_RANGE:this.data.creditBond.rate_start+'-'+this.data.creditBond.rate_end,//预期收益区间
      
      PF_PRODUCT_NAME:this.data.creditBond.PF_PRODUCT_NAME,//产品名称

      PF_ADMINISTRATOR:this.data.creditBond.PF_ADMINISTRATOR,//管理人
      PF_CUSTODIAN:this.data.creditBond.PF_CUSTODIAN,//托管人
      PF_PUBLISH_DEADLINE:this.data.creditBond.PF_PUBLISH_DEADLINE,//期限

      PF_SCALE:this.data.creditBond.PF_SCALE,//预期规模
      PF_PURCHASE_AMOUNT:this.data.creditBond.PF_PURCHASE_AMOUNT,//起购金额
      PF_SET_UP_DATE:this.data.creditBond.PF_SET_UP_DATE,//成立日期
      PF_DUE_DATE:this.data.creditBond.PF_DUE_DATE,//到期日期
      PF_OPEN_SEASON:this.data.mini_rate+'-'+this.data.high_rate,//开放期
      PF_INVESTMENT_SCOPE:this.data.creditBond.PF_INVESTMENT_SCOPE,//投资范围
    ...
    if(this.data.PF_ID){
       data.PF_ID=this.data.PF_ID
    }

    console.log(data)
    
    addOrUpdatePublicFund(data).then(res => {
      console.log(res);
      if(res.STATUS == 1){
        wx.showToast({
                title: '发布成功',
    ...
    if(options.info){
      ...
      if(info.PF_ID){
        ...
        let selectedItems={
         ...
         //  PF_OPEN_SEASON:this.data.mini_rate+'-'+this.data.high_rate,//开放期
          this.data.arr.map((item,index)=>{
            if(item==info.PF_INVESTMENT_TYPE){
              console.log(index)
          ...
          this.data.arr2.map((item2,index2)=>{
            ...
            if(item2==info.PF_OPERATION_MODE){
          ...
          //   if(item==info.PROJECT_TERM.split('-')[1]){
         ...
         'creditBond.PF_PRODUCT_NAME':info.PF_PRODUCT_NAME,//产品名称
   
         'creditBond.PF_ADMINISTRATOR':info.PF_ADMINISTRATOR,//管理人
         'creditBond.PF_CUSTODIAN':info.PF_CUSTODIAN,//托管人
         'creditBond.PF_PUBLISH_DEADLINE':info.PF_PUBLISH_DEADLINE,//期限
   
         'creditBond.PF_SCALE':info.PF_SCALE,//预期规模
         'creditBond.PF_PURCHASE_AMOUNT':info.PF_PURCHASE_AMOUNT,//起购金额
         'creditBond.PF_SET_UP_DATE':info.PF_SET_UP_DATE,//成立日期
         'creditBond.PF_DUE_DATE':info.PF_DUE_DATE,//到期日期
       
         'creditBond.PF_INVESTMENT_SCOPE':info.PF_INVESTMENT_SCOPE,//投资范围
        ...
        // current:info.REMARKS.length,
          ...
          if(info.PF_RETURN_RANGE){
            this.setData({
              'creditBond.rate_start':info.PF_RETURN_RANGE.split('-')[0],
              'creditBond.rate_end':info.PF_RETURN_RANGE.split('-')[1],
            })
          }   

          if(info.PF_OPEN_SEASON!=='-'){
            this.setData({
              mini_rate:info.PF_OPEN_SEASON.substring(0,10),
              high_rate:info.PF_OPEN_SEASON.substring(11,21),
            })
          }   
      }
