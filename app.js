...
App({
  ...
  globalData: {
    ...
    // CDNURL: 'https://app.jsbchina.cn/file/upload/wwcdn/wxapp',  //生产环境
    CDNURL: 'https://wxapptest.jsbchina.cn/rhedt',  //测试环境

    creditUrl: 'https://app.jsbchina.cn/file/upload/wwcdn/wxapp/',
  ...
  onLaunch() {
   ...
   //this.globalData.CDNURL = this.globalData.creditUrl;
    //初始化胶囊按钮
    this.loadStatusBar();
  ...
  getLocation: function () {
    ...
    return new Promise((resolve, reject) => {
      wx.getLocation({
        ...
        success: (res) => {
          return requestP({
              url: 'https://restapi.amap.com/v3/geocode/regeo?location=' +
                res.longitude +
                ',' +
                res.latitude +
  ...
  onShow: async function (options) {
    ...
    } else if (options.scene === 1038 || options.scene === 1068 || options.scene === 1090) {} else {
  ...
  viewHistoryShareInfo(shareList, options) {
    ...
    let data = {
      ...
      emp_no: that.globalData.sharerEmpNo == undefined || that.globalData.sharerEmpNo.length > 8 ?
        '' : that.globalData.sharerEmpNo,
    };
